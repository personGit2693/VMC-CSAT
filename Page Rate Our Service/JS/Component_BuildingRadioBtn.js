...
function BuildingRadioBtn(){
	...
	for(let index=0; index < buildingDetails_Array.length; index++){
		...
		buildingRadioBtn += `<div class="radioCheckFlex_RoClass">`+
			...
			`<input type="radio" id="`+elemValue+`" name="building-Name" value="`+elemValue+`" onchange="radioCheckSelected(this), valueBuilding(this.value), submitRequestFloors()" autocomplete="off"/>`+
			`<div class="radioCheckTxtLabel_RoClass"><label for="`+elemValue+`" style="cursor:pointer">`+buildingDetails_Array[index].building_name+`</label></div>`+
		`</div>`;
	}
