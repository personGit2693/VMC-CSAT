...
function RespondentTypeRadioBtn(){
	...
	for(let index=0; index < respondentDetails_Array.length; index++){
		...
		respondentTypeRadioBtn += `<div class="radioCheckFlex_RoClass">`+
			...
			`<input type="radio" id="`+elemValue+`" name="respondentType-Name" value="`+elemValue+`" onchange="radioCheckSelected(this), valueRespondentType(this.value), submitRequestOffices()" autocomplete="off"/>`+
