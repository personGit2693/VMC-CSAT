...
if(isset($_POST["token"]) && isset($_POST["clientTypeInternal"]) && isset($_POST["clientTypeExternal"]) && isset($_POST["officeId"]) && isset($_POST["dateFrom"]) && isset($_POST["dateTo"])){
	...
	if($serverConnection != null){
		echo json_encode($getCitizenCharterThreeScores_Resp, JSON_NUMERIC_CHECK);
		return;
	}
	/*Check connection*/
		...
		if((!empty($clientTypeInternal) || !empty($clientTypeExternal)) && (!empty($dateFrom) && !empty($dateTo))){		
			...
			$getCitizenCharterThreeScores_Query = "
				...
				FROM (SELECT ccquestionsrates_tab.ccquestionsrate_id AS 'ccquestionsrate_id',
					...
					WHERE ccquestionsrates_tab.ccquestion_id = 'CC3' 
			...
			$getCitizenCharterThreeScores_QueryObj->bindValue(':dateFrom', $dateFrom, PDO::PARAM_STR);
			$getCitizenCharterThreeScores_QueryObj->bindValue(':dateTo', $dateTo, PDO::PARAM_STR);
